...
class CifarResNeXtAux(nn.Module):
    """
    ResNext optimized for the Cifar dataset, as specified in
    https://arxiv.org/pdf/1611.05431.pdf
    """

    def __init__(self, cardinality, depth, num_classes, num_aux_classes,
        ...
        Args:
            ...
            widen_factor: factor to adjust the channel dimensionality
        """
        super(CifarResNeXtAux, self).__init__()
        self.cardinality = cardinality
        self.depth = depth
        self.block_depth = (self.depth - 2) // 9
        self.widen_factor = widen_factor
        ...
        self.stage_2 = self.block('stage_2', self.stages[1], self.stages[2], 2)
        self.stage_3 = self.block('stage_3', self.stages[2], self.stages[3], 2)
        self.classifier_cls = nn.Linear(1024, num_classes)
        self.classifier_aux = nn.Linear(1024, num_aux_classes)
        init.kaiming_normal(self.classifier_nor.weight)
        init.kaiming_normal(self.classifier_aux.weight)
...
def resnextAux(**kwargs):
    model = CifarResNeXtAux(**kwargs)
    return model
